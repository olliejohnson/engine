...
#if defined(_DEBUG)
    darray_push(required_extensions, &VK_EXT_DEBUG_UTILS_EXTENSION_NAME); // debug utilties
...
// and make sure they exist. Validation layers should only be enabled on non-release builds.
    ...
    // In flight fences should not exist yet at this point, so clear the list. These are stored in pointers
...
void vulkan_renderer_backend_on_resized(renderer_backend* backend, u16 width, u16 height) {
    // Update the "framebuffer size generation", a counter which indicated when the
...
b8 recreate_swapchain(renderer_backend* backend) {
    ...
    if (context.recreating_swapchain) {
        KDEBUG("recreate_swapchain called already recreating. Booting.");
        return false;
    }

    // Detect if the window is to small to be drawn to
