...
b8 vulkan_device_create(vulkan_context* context) {
    ...
    b8 transfer_shaders_graphics_queue = context->device.transfer_queue_index == context->device.graphics_queue_index;
    u32 index_count = 1;
    ...
    if (!transfer_shaders_graphics_queue) {
        index_count++;
    }
...
void vulkan_device_destroy(vulkan_context* context) {
    ...
    if (context->device.swapchain_support.present_modes) {
        kfree(
            ...
            sizeof(VkSurfacePresentModeKHR) * context->device.swapchain_support.present_mode_count,
            MEMORY_TAG_RENDERER);
